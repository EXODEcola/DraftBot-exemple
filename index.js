...
const activities = [
  "https://site.exode-hebergement.fr",
  "Default Prefix : +",
  "Server : https://discord.gg/tstMQSxQwU",
  "🐢",
];
...
client.on("ready", () => {
  ...
  let readychannel = client.channels.cache.get("1103407508367876116");
  const ready = new Discord.MessageEmbed()
    .setTitle("Yoohoo Je suis en Ligne :white_check_mark:")
    .setThumbnail(
      client.user.displayAvatarURL({ format: "png", dynamic: true, size: 1024 })
    )
    .setDescription(
      `\n **${user}** *Nombre de Membres* \n **${guild}** *servers*  \n**${channel}** *Nombres de channels***`
    )
  ...
  const DevEvil = String.raw`
   ...
   ________                   ________      _________      ________      _________       ________   ___           ___
  |  ______|   ___    ___    |   __   |    |   ___   \    |  ______|    |   ___   \     |  ______|  \  \         /  /
  |  _____     \  \  /  /    |  |  |  |    |  |   |  |    |  _____      |  |   |  |     |  _____     \  \       /  /
  |  _____|     \  \/  /     |  |  |  |    |  |   |  |    |  _____|     |  |   |  |     |  _____|     \  \     /  /
  |  ______     /  /\  \     |  |__|  |    |  |___|  |    |  ______     |  |___|  |     |  ______      \  \___/  /
  |________|   /__/  \__\    |________|    |_________/    |________|    |_________/     |________|      \_______/
                                                   
                                                   
      EXODE BOTING is online               
      Developer: EXODEcola ✪#7410
...
client.on("guildCreate", (guild) => {
  ...
  const joinembed = new Discord.MessageEmbed()
    .setAuthor("Hello, I'm EXODE BOTING", client.user.displayAvatarURL())
    .setDescription(
      "**Merci de m’avoir invité sur votre serveur :grin: :heart:** n**Mentionnez-moi pour obtenir de l’aide ou tapez ''+help''** n**Rejoignez notre serveur pour des nouvelles des mises à jour du bot en tapant ''+server''** n**Votre préfixe de serveur : ''+''** n**Vous pouvez me soutenir par vos upvotes ''+upvote''** n**Vous pouvez changer le préfixe du bot en tapant ''+prefix''**"
    )
    .setThumbnail(
      client.user.displayAvatarURL({ format: "png", dynamic: true, size: 1024 })
    )
...
client.on("guildCreate", (guild) => {
  if (!guild.partial) {
    const channel = client.channels.cache.get("1103407508367876116");
    if (channel) {
